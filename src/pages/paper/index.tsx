...
export default () => {
  ...
  } = chHooks.useOptionFormListHook({ url: 'http://api-paper.kukechen.top/api/subject/list' })
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'subjectId',
      render: (text: string, record: Paper) => {
        return <div>{record.tagIds.map(subjectId => {
          return <span key={subjectId}>{subjectOptionMap[subjectId] && tagOptionMap[subjectId].name}</span>
        })}</div>
      }
    },
    {
          ...
          [
            {
              ...
              rules: [{
                required: true, message: '文件上传失败', validator:
                  (rule, value, callback) => {
                    try {
                      if (value[0].response) {
